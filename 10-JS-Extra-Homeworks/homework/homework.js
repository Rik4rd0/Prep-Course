...
function numberOfCharacters(string) {
  ...
  //Escribe tu código aquí
  //hasOwnpropery verifica la propiedad string existe.
  var obj={};
...
function asAmirror(str) {
  ...
  //Escribe tu código aquí
   var conv =str.split(' ');//[t,h,e,h,e,n,r,y]   [yhe, henry, challenge]
   let rta=[];
...
function capicua(numero){
  ...
  //Escribe tu código aquí
    var num =numero.toString();//549 > "540" 
    // '045' = '540'  -> ['5','4','0'] -> ['0'.'4','5'] -> '045'
   ...
   while (cambio) {
     ...
     for (var i = 0; i < arr.length-1; i++) {
            if (arr[i].length > arr[i+1].length) {//["You", "are", "beautiful", "looking"]
              var aux=arr[i]; // aux = 'beutuful'
              arr[i]=arr[i+1];//= arr[i]='lookin'
              arr[i+1] = aux;//arr[i] =beutiful
             cambio=true; 
            }       
     }
